...
export async function middleware(req: NextRequest) {
  ...
  let session;
  const impersonatedSessionCookie = req.cookies.get('impersonated_session_payload');

  if (impersonatedSessionCookie?.value) {
    try {
      // کوکی را قبل از پارس کردن، کدگشایی (decode) می‌کنیم
      const decodedCookie = decodeURIComponent(impersonatedSessionCookie.value);
      session = JSON.parse(decodedCookie);
    } catch {
