...
export const AccountHeaderCard: FC<Props> = ({ role, locale = "fa", user, validityText }) => {
  ...
  return (
    <section
      dir="rtl"
      className="relative overflow-hidden rounded-3xl border border-purple-200 bg-gradient-to-br from-[#EBD9FF] via-[#EFE8FF] to-[#E7F2FF] p-4 shadow-[0_12px_34px_-12px_rgba(109,40,217,.35)]"
    >
      {/* آواتار بالا-راست */}
      ...
      {/* بدنه: کاملاً راست‌چین زیر آواتار */}
      <div className="mt-2 text-right">
        ...
        {/* تلفن‌ها: از سمت راست، آیکن تلفن سپس شماره‌ها با جداکننده عمودی */}
            ...
            <div className="flex items-center">
              {phones.map((ph, i) => (
                <span key={i} className="flex items-center">
                  ...
                  {i < phones.length - 1 && <span className="mx-2 inline-block h-3 w-px bg-gray-300" />}
                </span>
              ))}
            </div>
          </div>
      ...
      {/* شبکه‌های اجتماعی: وسط‌چین */}
      {isWholesale && (
        <div className="mt-4 flex items-center justify-center gap-8 text-gray-700">
          {user.instagramUrl && (
