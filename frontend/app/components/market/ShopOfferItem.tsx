...
function absolutizeUploads(u?: string | null) {
  ...
  const host = (
    process.env.NEXT_PUBLIC_FILE_HOST || "https://nerkhin.com"
  ).replace(/\/+$/, "");
  const prefix =
    "/" +
    (process.env.NEXT_PUBLIC_FILE_PREFIX || "uploads").replace(/^\/+/, "");
...
export default function ShopOfferItem({ t, item }: Props) {
  ...
  return (
    <div className="py-3" dir="rtl">
      {/* ردیف بالا: [چپ=تاریخ] [وسط=متن] [راست=عکس] */}
      ...
      {/* ردیف پایین: [راست=تلفن] [میان‌خط] [وسط=تعداد لایک] [چپ=ستاره] */}
      <div className="mt-2  flex pt-2 border-t border-gray-100  ">
        ...
        <div className="flex items-center gap-2 w-full justify-center">
          <div className="flex items-center gap-2 text-sm text-gray-700">
            {(
              [item.shopPhone1, item.shopPhone2, item.shopPhone3].filter(
                Boolean
              ) as string[]
            )
              .slice(0, 1)
