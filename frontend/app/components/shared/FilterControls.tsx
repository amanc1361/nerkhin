...
export default function FilterControls({
  messages, brands = [], categories = [], subCategories = [], initial, onChange,
}: Props) {
  ...
  useEffect(() => {
    const payload: FilterControlsValue = {
      brandIds, categoryId, subCategoryId, isDollar, sortUpdated, search,
    };
    onChange(payload);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <label className="text-sm">
          ...
          >
            ...
            {brands.map(b => <option key={b.value} value={String(b.value)}>{b.label}</option>)}
          </select>
        </label>
        ...
        <label className="text-sm">
          ...
          >
            ...
            {categories.map(c => <option key={c.value} value={String(c.value)}>{c.label}</option>)}
          </select>
        </label>
        ...
        <label className="text-sm">
          ...
          >
            <option value="">{t.brandAll}</option>
            {subCategories.map(sc => <option key={sc.value} value={String(sc.value)}>{sc.label}</option>)}
          </select>
        </label>
      </div>

      {/* سطر ۲: نوع قیمت + مرتب‌سازی */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <label className="text-sm">
