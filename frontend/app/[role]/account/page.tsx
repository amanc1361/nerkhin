...
export default async function AccountPage({ params }: { params: { role: string } }) {
  const roleSegment = (params.role === "wholesaler" ? "wholesaler" : "retailer") as "wholesaler" | "retailer";
  ...
  return (
    <main className="mx-auto max-w-screen-sm px-3 py-4">
      ...
      <div className="mt-4 space-y-3">
        <AccountListItem href={`/${roleSegment}/likes`}        title={t.list.likes}        icon={<span>⭐</span>} />
        {isWholesale && (
          <AccountListItem href={`/${roleSegment}/customers`}   title={t.list.customers}    icon={<span>👥</span>} />
        )}
        <AccountListItem href={`/${roleSegment}/transactions`} title={t.list.transactions} icon={<span>💳</span>} />
